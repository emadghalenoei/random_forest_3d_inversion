# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 14:59:54 2020
...
def Gravity_3D_Kernel_Expanded_MD(X,Y,Z,XS,YS,dx,dy,dz):
    ...
    """
    # flatten('F') vectorizes X Y Z like MATLAB X(:), [col1 ; col2 ; ...]
    # flatten('C') vectorizes X Y Z as: [row1 , row2 , ...]
    # Thus: The order of Kernel must be identical to order of vectorized Model
    # DensityModel.flatten('C')
    
    # it is easeir to use flatten('C') in the Kernel as the 3D model cannot
    # be vectorized properly by F
    """
    
    # Ximat = np.diag(XS.flatten('F')) @ A
    # Yimat = np.diag(YS.flatten('F')) @ A
    # Xjmat = A @ np.diag(X.flatten('F'))
    # Yjmat = A @ np.diag(Y.flatten('F'))
    # Zjmat = A @ np.diag(Z.flatten('F'))
